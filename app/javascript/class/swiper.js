export class Swiper { constructor(attributes) {
  ...
  currentCardEventsCallback(e) {
    console.log('jai clique sur la catrter')
  ...
  release() {
    ...
    if (Math.abs(this.pullDeltaX) >= this.threshold) {
      console.log('swiope jusquau bout')
      const id = this.currentCard.dataset.id
      like ? this.likedCallback(id) : this.dislikedCallback(id)
      ...
      console.log('remet la carte ou elle etait frere')
